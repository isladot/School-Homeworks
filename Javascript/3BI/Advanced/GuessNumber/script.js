...
function check(){
    ...
    if(cont<=nmosse && found!=true){
        if(num < random){
            ...
            var newSpan = document.createElement("span");
            newSpan.innerHTML = "Mossa " + cont + ": il numero " + num + " è maggiore!";
            results.appendChild(newSpan);
            var newBr = document.createElement("br");
            results.appendChild(newBr);
            cont++;
            ...
            found = true;
            document.getElementById("risposta").innerHTML = random + ", hai indovinato in " + cont + " mosse!";
        }
    } 
    if(cont>nmosse){
        document.getElementById("risposta").innerHTML = random + ", non hai indovinato in " + nmosse + " mosse!"; 
        alert("Hai finito il numero di mosse disponibili! Resetta il gioco per riprovare.");
    }
}
